...
public class SqliteDatabase extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "farmer";
    private static final String TABLE_PRODUCTS = "farmer";
    ...
    public List<Farmer_ProductModel> listProducts(){
        String sql = "select * from " + TABLE_PRODUCTS;
        android.database.sqlite.SQLiteDatabase db = this.getReadableDatabase();
        List<Farmer_ProductModel> storeProducts = new ArrayList<>();
        ...
        if(cursor.moveToFirst()){
            do{
                ...
                storeProducts.add(new Farmer_ProductModel(id, pro_id,pro_name, price,img,qty));
    ...
    public List<Ferti_Pesti_Buying> listProductsFerti(){
        String sql = "select * from " + TABLE_PRODUCTS;
        android.database.sqlite.SQLiteDatabase db = this.getReadableDatabase();
        List<Ferti_Pesti_Buying> storeProducts = new ArrayList<>();
        Cursor cursor = db.rawQuery(sql, null);
        if(cursor.moveToFirst()){
            do{
                int id = Integer.parseInt(cursor.getString(0));
                ...
                storeProducts.add(new Ferti_Pesti_Buying(id, pro_id,pro_name, price,img,qty));
            }while (cursor.moveToNext());
        }
        cursor.close();
        return storeProducts;
    ...
    public void addProduct(Farmer_ProductModel product){
    ...
    public void addProductFerti(Ferti_Pesti_Buying product){
        ContentValues values = new ContentValues();
        values.put(COLUMN_BRANDNAME, product.getName());
        values.put(COLUMN_BRANDLOGO, product.getImg());
        values.put(COLUMN_BRANDID, product.getId());
        values.put(COLUMN_BRANDABOUT, product.getPrice());
        values.put(COLUMN_QTY, product.getQty());
